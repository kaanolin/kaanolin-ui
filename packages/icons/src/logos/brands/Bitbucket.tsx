// [brand] generated from plugin-misc-icons.json
import { forwardRef, useId } from "react";
import type { IconBaseProps } from "../../types";

export const Bitbucket = forwardRef<SVGSVGElement, IconBaseProps>(function Bitbucket(
  { size = 24, title, "aria-label": ariaLabel, ...rest },
  ref,
) {
  const uid = useId().replace(/[^a-zA-Z0-9_-]/g, "");
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 32 32"
      fill="none"
      role={title || ariaLabel ? "img" : "presentation"}
      aria-label={ariaLabel}
      aria-hidden={title || ariaLabel ? undefined : true}
      {...rest}
    >
      {title ? <title>{title}</title> : null}
      <path d="M2.9087 3.00008C2.64368 2.99655 2.3907 3.11422 2.21764 3.32152C2.04458 3.52883 1.96915 3.80455 2.01158 4.07472L5.81987 27.9484C5.91782 28.5515 6.42093 28.9949 7.01305 28.9999H25.283C25.7274 29.0058 26.109 28.6748 26.1801 28.2217L29.9884 4.07935C30.0309 3.80918 29.9554 3.53346 29.7824 3.32615C29.6093 3.11885 29.3563 3.00118 29.0913 3.00471L2.9087 3.00008ZM18.9448 20.2546H13.1135L11.5346 11.7362H20.3578L18.9448 20.2546Z" fill="#2684FF"/>
<path fillRule="evenodd" clipRule="evenodd" d="M28.7778 11.7362H20.3582L18.9453 20.2546H13.114L6.22852 28.6942C6.44675 28.8891 6.725 28.9975 7.0135 29H25.2879C25.7324 29.0059 26.114 28.6748 26.1851 28.2218L28.7778 11.7362Z" fill={`url(#${uid}-paint0_linear_1334_1409)`}/>
<defs>
<linearGradient id={`${uid}-paint0_linear_1334_1409`} x1="30.7245" y1="14.1217" x2="20.5764" y2="28.0751" gradientUnits="userSpaceOnUse">
<stop offset="0.18" stopColor="#0052CC"/>
<stop offset="1" stopColor="#2684FF"/>
</linearGradient>
</defs>
    </svg>
  );
});
Bitbucket.displayName = "Bitbucket";
