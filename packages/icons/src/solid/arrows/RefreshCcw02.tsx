// [solid] generated from source/icons/Solid icons/
import { forwardRef } from "react";
import type { SolidIconProps } from "../../types";

export const RefreshCcw02 = forwardRef<SVGSVGElement, SolidIconProps>(function RefreshCcw02(
  { size = 24, color = "currentColor", title, "aria-label": ariaLabel, ...rest },
  ref,
) {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      color={color}
      role={title || ariaLabel ? "img" : "presentation"}
      aria-label={ariaLabel}
      aria-hidden={title || ariaLabel ? undefined : true}
      {...rest}
    >
      {title ? <title>{title}</title> : null}
      <path d="M19.0711 4.92893C15.1659 1.02369 8.83421 1.02369 4.92897 4.92893C4.13086 5.72704 3.49967 6.39522 3 6.95364V4C3 3.44772 2.55228 3 2 3C1.44772 3 1 3.44772 1 4V10C1 10.5523 1.44772 11 2 11H8C8.55228 11 9 10.5523 9 10C9 9.44772 8.55228 9 8 9H3.87433C4.38652 8.38281 5.16508 7.52125 6.34318 6.34315C9.46738 3.21895 14.5327 3.21895 17.6569 6.34315C18.7649 7.45114 19.4787 8.80057 19.801 10.2213C19.9232 10.7599 20.4589 11.0974 20.9975 10.9752C21.5361 10.853 21.8736 10.3173 21.7514 9.77873C21.348 8.00086 20.4537 6.31149 19.0711 4.92893Z" fill="currentColor"/>
<path d="M4.19906 13.7787C4.07686 13.2401 3.54118 12.9026 3.00258 13.0248C2.46399 13.147 2.12643 13.6826 2.24864 14.2212C2.65202 15.9991 3.5464 17.6885 4.92896 19.071C8.8342 22.9763 15.1658 22.9763 19.0711 19.071C19.8689 18.2733 20.5001 17.6051 21 17.0464V20C21 20.5523 21.4477 21 22 21C22.5523 21 23 20.5523 23 20V14C23 13.4477 22.5523 13 22 13H16C15.4477 13 15 13.4477 15 14C15 14.5523 15.4477 15 16 15H20.1257C19.6142 15.6164 18.8358 16.4779 17.6569 17.6568C14.5327 20.781 9.46736 20.781 6.34317 17.6568C5.23518 16.5488 4.52141 15.1994 4.19906 13.7787Z" fill="currentColor"/>
    </svg>
  );
});
RefreshCcw02.displayName = "RefreshCcw02";
