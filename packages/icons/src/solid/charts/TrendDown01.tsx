// [solid] generated from source/icons/Solid icons/
import { forwardRef } from "react";
import type { SolidIconProps } from "../../types";

export const TrendDown01 = forwardRef<SVGSVGElement, SolidIconProps>(function TrendDown01(
  { size = 24, color = "currentColor", title, "aria-label": ariaLabel, ...rest },
  ref,
) {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      color={color}
      role={title || ariaLabel ? "img" : "presentation"}
      aria-label={ariaLabel}
      aria-hidden={title || ariaLabel ? undefined : true}
      {...rest}
    >
      {title ? <title>{title}</title> : null}
      <path d="M2.70711 6.29289C2.31658 5.90237 1.68342 5.90237 1.29289 6.29289C0.902369 6.68342 0.902369 7.31658 1.29289 7.70711L6.18407 12.5983C6.36251 12.7768 6.53629 12.9506 6.69551 13.0858C6.87036 13.2342 7.09225 13.3938 7.38197 13.4879C7.78365 13.6184 8.21635 13.6184 8.61803 13.4879C8.90775 13.3938 9.12964 13.2342 9.30449 13.0858C9.4637 12.9506 9.63748 12.7768 9.81592 12.5983L12.5757 9.83848C12.7854 9.6288 12.9007 9.51462 12.9899 9.43892L13 9.4304L13.0101 9.43892C13.0993 9.51462 13.2146 9.6288 13.4243 9.83848L19.5858 16H15C14.4477 16 14 16.4477 14 17C14 17.5523 14.4477 18 15 18H22C22.5523 18 23 17.5523 23 17V10C23 9.44772 22.5523 9 22 9C21.4477 9 21 9.44772 21 10V14.5858L14.8159 8.40171C14.6375 8.22323 14.4637 8.0494 14.3045 7.91424C14.1296 7.76581 13.9078 7.60624 13.618 7.5121C13.2163 7.38158 12.7837 7.38158 12.382 7.5121C12.0922 7.60624 11.8704 7.76581 11.6955 7.91424C11.5363 8.04939 11.3625 8.2232 11.1841 8.40166L8.42426 11.1615C8.21459 11.3712 8.09931 11.4854 8.01013 11.5611L8 11.5696L7.98987 11.5611C7.90069 11.4854 7.78541 11.3712 7.57574 11.1615L2.70711 6.29289Z" fill="currentColor"/>
    </svg>
  );
});
TrendDown01.displayName = "TrendDown01";
